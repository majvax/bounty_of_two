...
public:
    ...
    bool connect()
    {
        ...
        if (status != sf::Socket::Status::Done) {
            logger->critical("Failed to connect to server at {}:{}", server_address.toInteger(), TCP_SERVER_PORT);
            return false;
        }
        ...
        if (status != sf::Socket::Status::Done) {
            logger->critical("Failed to bind UDP socket to port {}", UDP_SELF_PORT);
            return false;
        }

        logger->info("Connected to server at {}:{}", server_address.toInteger(), TCP_SERVER_PORT);
        ...
        if (status != sf::Socket::Status::Done) {
            logger->error(
              "Failed to send JoinNotification to server at {}:{}", server_address.toInteger(), TCP_SERVER_PORT);
            return false;
        }
    ...
    void disconnect()
    {
        ...
        if (tcp_socket.send(packet) != sf::Socket::Status::Done) {
            logger->error(
              "Failed to send LeaveNotification to server at {}:{}", server_address.toInteger(), TCP_SERVER_PORT);
        }
    ...
    void send_message(const net::packet_t<Msg>& packet)
    {
        const auto logger = spdlog::get("network");
        if (tcp_socket.send(packet) != sf::Socket::Status::Done) {
            logger->error("Failed to send packet to server at {}:{}", server_address.toInteger(), TCP_SERVER_PORT);
        }
    }
    ...
    {
        const auto logger = spdlog::get("network");
        const auto status = udp_socket.send(packet, server_address, UDP_SERVER_PORT);
        if (status != sf::Socket::Status::Done) {
            logger->error("Failed to send packet to server at {}:{}", server_address.toInteger(), UDP_SERVER_PORT);
        }
    }
